...
% endif
">
        <h3><a href="chats/${chat.url}/1.html">${chat_user.display_title}</a>\
% if chat.updated>chat_user.visited:
 (unread)\
...
% endif
Started ${request.user.localise_time(chat.created).strftime("%a %d %b %Y")}, last message ${request.user.localise_time(chat.updated).strftime("%a %d %b %Y")}.</p>
        ...
        <p class="notes">From request #${chat.request_id}</p>
        % endif
        % if chat_user.notes!="":
