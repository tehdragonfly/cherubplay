    ...
    <ul id="messages" class="tile2">
      ...
      % if request.context.chat.mode != ChatMode.group:
        % for banned_chat_user in request.context.banned_chat_users:
          <li class="message message_system"><i>${banned_chat_user.handle}</i> has been ${"temporarily" if banned_chat_user.user.unban_date else "permanently"} banned from Cherubplay.</li>
        % endfor
        % for away_chat_user in request.context.away_chat_users:
          % if away_chat_user not in request.context.banned_chat_users and away_chat_user != request.context.chat_user:
            <li class="message message_system"><p><i>${away_chat_user.handle}</i> has marked their account as away. They left this message:
