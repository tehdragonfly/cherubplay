...
% if prompt:
  <section class="tile2">
    <h3>Prompt</h3>
    <div class="delete_preview message message_${prompt.type.value}${" edited" if prompt.show_edited else ""}" data-symbol="${prompt.symbol_character if prompt.symbol is not None else ""}" style="color: #${prompt.colour};">
...
% if last_message and last_message != prompt:
  <section class="tile2">
    <h3>Last message</h3>
    <div class="delete_preview message message_${last_message.type.value}${" edited" if last_message.show_edited else ""}" data-symbol="${last_message.symbol_character if last_message.symbol is not None else ""}" style="color: #${last_message.colour};">
