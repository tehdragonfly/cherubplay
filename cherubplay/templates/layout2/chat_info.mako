...
% if request.context.chat.status == "ongoing" and request.context.chat.mode == ChatMode.group:
  <section class="tile2">
    <h3>Dramatis personae</h3>
    <ul class="tag_list">
      % for user_id, chat_user in request.context.chat_users.items():
        <li style="color: #${chat_user.last_colour}">
          % if chat_user.user_id == request.user.id and request.has_permission("chat.change_name"):
