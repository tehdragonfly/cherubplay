...
${"TW:&nbsp;" if tag.tag.type == "trigger" else ""|n}${tag.tag.name}\
% else:
<a href="${request.route_path("directory_tag", type=tag.tag.type, name=tag.tag.url_name)}">${"TW:&nbsp;" if tag.tag.type == "trigger" else ""|n}${tag.tag.name}</a>\
% endif
</li>\
</%def>
